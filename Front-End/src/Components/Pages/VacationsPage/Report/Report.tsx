...
function Report(): JSX.Element {
    ...
    const followersCount = vacations.map((vacation) => ({
        vacation_key: vacation.vacation_key,
        followers: followers.filter(
            (follower) => follower.vacation_key === vacation.vacation_key
        ).length,
    }));

    // Create the data array for the chart
    const data = vacations.map((vacation) => ({
        Destination: vacation.destination.split("-")[0],
        Followers:
            followersCount.find(
                (follower) => follower.vacation_key === vacation.vacation_key
            )?.followers || 0,
